...
def test_horizontal_merge(vcf_merger, unique_samples_vcfs):
    ...
    assert filenames == [os.path.join(vcf_merger.output_dir, 'alias_merged.vcf.gz')]
    assert_all_files_present(filenames)
...
def test_horizontal_merge_multiple_groups(vcf_merger, unique_samples_vcfs, unique_samples_vcfs_2):
    ...
    assert sorted(filenames) == [
        os.path.join(vcf_merger.output_dir, '1_merged.vcf.gz'),
        os.path.join(vcf_merger.output_dir, '2_merged.vcf.gz')
    ]
    assert_all_files_present(filenames)
